# StarUML Controller MCP Server — Timing Diagram

An MCP (Model Context Protocol) server specialized for **Timing Diagrams**. Enables AI assistants like Claude to programmatically create and manage timing diagrams — lifelines, timing states, and time segments in [StarUML](https://staruml.io/).

<p align="center">
  <img src="images/main.jpg" alt="Elevator Control System — Timing Diagram created with staruml-controller-timing-mcp" width="800">
</p>

> *Elevator Control System timing diagram — created entirely through MCP tool calls by Claude*

## Architecture

<p align="center">
  <img src="images/architecture.svg" alt="Architecture: Claude ↔ MCP (stdio) ↔ staruml-controller-timing-mcp ↔ HTTP REST API ↔ StarUML" width="800">
</p>
...
```bash
git clone https://github.com/pontasan/staruml-controller-timing-mcp.git
cd staruml-controller-timing-mcp
npm install && npm run build
```
...
4. The HTTP server starts — all Timing Diagram tools become available via MCP
...
```json
{
  "mcpServers": {
    "staruml-timing": {
      "command": "node",
      "args": ["/absolute/path/to/staruml-controller-timing-mcp/dist/index.js"]
    }
  }
}
```
...
```bash
claude mcp add staruml-timing node /absolute/path/to/staruml-controller-timing-mcp/dist/index.js
```
...
The core of this server — full CRUD for every Timing Diagram resource.

| Resource | Tools |
|---|---|
| **Diagrams** | `timing_list_diagrams`, `timing_create_diagram`, `timing_get_diagram`, `timing_update_diagram`, `timing_delete_diagram` |
| **Lifelines** | `timing_list_lifelines`, `timing_create_lifeline`, `timing_get_lifeline`, `timing_update_lifeline`, `timing_delete_lifeline` |
| **Timing States** | `timing_list_timing_states`, `timing_create_timing_state`, `timing_get_timing_state`, `timing_update_timing_state`, `timing_delete_timing_state` |
| **Time Segments** | `timing_list_time_segments`, `timing_create_time_segment`, `timing_get_time_segment`, `timing_update_time_segment`, `timing_delete_time_segment` |
